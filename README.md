...
- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  ...
  actions are called and sent to the reducer to update the store, or central state of the app, which renders with the new state. It is the single source of truth because all of the main state resides in it. actions are just objects that if match the case in a reducer, signify what logic to do. the reducer is what actually updates the state.

- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?

  application state is state residing in the app, and component state is when state is in localized components?

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

  redux-thunk is a middleware that allows our action creators to perform asynchronously.
